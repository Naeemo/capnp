...
echo "=== Phase 2 Interop Test Data Generation ==="
...
capnp compile -oc++ test-schema-phase2.capnp 2>/dev/null || echo "Schema 2 compilation skipped (may need manual C++ compilation)"
...
EOF
capnp convert text:binary test-schema-phase2.capnp GroupType < /tmp/group_type.txt > data/group_type.bin 2>/dev/null || echo "GroupType: skipped (needs C++ generator)"
...
()
EOF
capnp convert text:binary test-schema-phase2.capnp EmptyStruct < /tmp/empty_struct.txt > data/empty_struct.bin 2>/dev/null || echo "EmptyStruct: skipped (needs C++ generator)"

# Data only struct
cat > /tmp/data_only.txt << 'EOF'
(id = 123456789012345, count = 42, flag = true)
EOF
capnp convert text:binary test-schema-phase2.capnp DataOnlyStruct < /tmp/data_only.txt > data/data_only_struct.bin 2>/dev/null || echo "DataOnlyStruct: skipped (needs C++ generator)"

# Pointer only struct
cat > /tmp/pointer_only.txt << 'EOF'
(name = "Test Name", data = 0x"DEADBEEF")
EOF
capnp convert text:binary test-schema-phase2.capnp PointerOnlyStruct < /tmp/pointer_only.txt > data/pointer_only_struct.bin 2>/dev/null || echo "PointerOnlyStruct: skipped (needs C++ generator)"
