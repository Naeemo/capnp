...
describe('Edge Cases - Empty Messages', () => {
  ...
  it('should handle message with empty root struct', () => {
    ...
    const root = builder.initRoot(0, 0); // 0 data words, 0 pointers
...
describe('Edge Cases - Numeric Limits', () => {
  ...
  it('should handle Float32 special values', () => {
    ...
    root.setFloat32(0, Infinity);
    root.setFloat32(4, -Infinity);
    root.setFloat32(8, NaN);
    
    const buffer = builder.toArrayBuffer();
    const readRoot = new MessageReader(buffer).getRoot(2, 0);
    
    expect(readRoot.getFloat32(0)).toBe(Infinity);
    expect(readRoot.getFloat32(4)).toBe(-Infinity);
  ...
  it('should handle Float64 special values', () => {
    ...
    root.setFloat64(0, Infinity);
    root.setFloat64(8, -Infinity);
    root.setFloat64(16, NaN);
    
    const buffer = builder.toArrayBuffer();
    const readRoot = new MessageReader(buffer).getRoot(4, 0);
    
    expect(readRoot.getFloat64(0)).toBe(Infinity);
    expect(readRoot.getFloat64(8)).toBe(-Infinity);
