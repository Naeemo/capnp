 ...
 * 编码 Struct 指针
 */
export function encodeStructPointer(offset: number, dataWords: number, pointerCount: number): bigint {
  const offsetBits = BigInt(offset < 0 ? offset + 0x40000000 : offset) & BigInt(0x3FFFFFFF);
  return (offsetBits << BigInt(2)) | 
 ...
 * 编码 List 指针
 */
export function encodeListPointer(offset: number, elementSize: ElementSize, elementCount: number): bigint {
  const offsetBits = BigInt(offset < 0 ? offset + 0x40000000 : offset) & BigInt(0x3FFFFFFF);
  return (offsetBits << BigInt(2)) | 
         BigInt(1) |  // LIST tag
 ...
 * 编码 Far 指针
 */
export function encodeFarPointer(segment: number, offset: number, doubleFar: boolean = false): bigint {
  const offsetBits = BigInt(offset) & BigInt(0x1FFFFFFF);
  const segmentBits = BigInt(segment) & BigInt(0xFFFFFFFF);
  
  return (segmentBits << BigInt(32)) |
         (offsetBits << BigInt(3)) |
