...
export function encodeCapabilities(capabilities: TransportCapabilities): Uint8Array {
  ...
  const algoList = root.initList<number>(0, ElementSize.BYTE, capabilities.compressionAlgorithms.length);
...
export function negotiateCompression(
  ...
  // 检查算法列表有效性
  if (!localCapabilities.compressionAlgorithms || localCapabilities.compressionAlgorithms.length === 0) {
    return {
  ...
  if (!remoteCapabilities.compressionAlgorithms || remoteCapabilities.compressionAlgorithms.length === 0) {
    return {
      selectedAlgorithm: CompressionAlgorithm.NONE,
      success: true,
      error: undefined,
