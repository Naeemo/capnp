...
class SchemaParser {
  ...
  private parseNode(offset: number): SchemaNode {
    ...
    const nestedNodes = nestedNodesPtr.offset > 0 && nestedNodesPtr.size > 0
      ? this.readStructList(nestedNodesPtr.offset, nestedNodesPtr.size, (noffset) => this.parseNestedNode(noffset))
      : [];
    
    // Read annotations (pointer at offset 40)
    const annotationsPtr = this.readPointer(offset + 40);
    const annotations = annotationsPtr.offset > 0 && annotationsPtr.size > 0
      ? this.readStructList(annotationsPtr.offset, annotationsPtr.size, (aoffset) => this.parseAnnotation(aoffset))
      : [];
  ...
  private parseStructInfo(offset: number): NonNullable<SchemaNode["structInfo"]> {
    ...
    const fields = fieldsPtr.offset > 0 && fieldsPtr.size > 0
      ? this.readStructList(fieldsPtr.offset, fieldsPtr.size, (foffset) => this.parseField(foffset))
      : [];
  ...
  private parseField(offset: number): SchemaField {
    ...
    const defaultValue = defaultValuePtr.offset > 0 ? this.parseValue(defaultValuePtr.offset) : undefined;
  ...
  private parseBrand(offset: number): SchemaBrand {
    ...
    const scopes = scopesPtr.offset > 0 && scopesPtr.size > 0
      ? this.readStructList(scopesPtr.offset, scopesPtr.size, (soffset) => this.parseBrandScope(soffset))
      : [];
  ...
  private parseBrandScope(offset: number): SchemaBrandScope {
    ...
    if (discriminant === 0) {
      ...
      bindings = bindingsPtr.offset > 0 && bindingsPtr.size > 0
        ? this.readStructList(bindingsPtr.offset, bindingsPtr.size, (boffset) => this.parseBrandBinding(boffset))
        : [];
    }
  ...
  private parseValue(offset: number): SchemaValue {
    ...
    switch (discriminant) {
      case 0: return { type: "void" };
      case 1: return { type: "bool", value: this.view.getUint8(dataOffset) !== 0 };
      case 2: return { type: "int8", value: this.view.getInt8(dataOffset) };
      case 3: return { type: "int16", value: this.view.getInt16(dataOffset, true) };
      case 4: return { type: "int32", value: this.view.getInt32(dataOffset, true) };
      case 5: return { type: "int64", value: this.view.getBigInt64(dataOffset, true) };
      case 6: return { type: "uint8", value: this.view.getUint8(dataOffset) };
      case 7: return { type: "uint16", value: this.view.getUint16(dataOffset, true) };
      case 8: return { type: "uint32", value: this.view.getUint32(dataOffset, true) };
      case 9: return { type: "uint64", value: this.view.getBigUint64(dataOffset, true) };
      case 10: return { type: "float32", value: this.view.getFloat32(dataOffset, true) };
      case 11: return { type: "float64", value: this.view.getFloat64(dataOffset, true) };
      case 12: {
      ...
      case 14: return { type: "list", value: null }; // Simplified
      case 15: return { type: "enum", value: this.view.getUint16(dataOffset, true) };
      case 16: return { type: "struct", value: null }; // Simplified
      case 17: return { type: "interface" };
      case 18: return { type: "anyPointer", value: null }; // Simplified
      default:
        return { type: "void" };
    }
  }
  ...
  private parseEnumInfo(offset: number): NonNullable<SchemaNode["enumInfo"]> {
    ...
    const enumerants = enumerantsPtr.offset > 0 && enumerantsPtr.size > 0
      ? this.readStructList(enumerantsPtr.offset, enumerantsPtr.size, (eoffset) => this.parseEnumerant(eoffset))
      : [];
   ...
   * Parse enumerant
   */
  private parseEnumerant(offset: number): { name: string; codeOrder: number; annotations: SchemaAnnotation[] } {
    // Enumerant layout:
    // - name: Text @0
    // - codeOrder: UInt16 @1
    // - annotations: List(Annotation) @2
  ...
  private parseInterfaceInfo(offset: number): NonNullable<SchemaNode["interfaceInfo"]> {
    ...
    const methods = methodsPtr.offset > 0 && methodsPtr.size > 0
      ? this.readStructList(methodsPtr.offset, methodsPtr.size, (moffset) => this.parseMethod(moffset))
      : [];
    
    const superclassesPtr = this.readPointer(offset + 8);
    const superclasses = superclassesPtr.offset > 0 && superclassesPtr.size > 0
      ? this.readStructList(superclassesPtr.offset, superclassesPtr.size, (soffset) => this.parseSuperclass(soffset))
      : [];
  ...
  private readPointer(offset: number): { offset: number; size: number } {
    ...
    const type = (sizeAndType >> 24) & 0x07;
   ...
   */
  private readStructList<T>(
    offset: number, 
    count: number, 
    parser: (offset: number) => T
  ): T[] {
    const result: T[] = [];
